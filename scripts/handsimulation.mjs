...
export async function startHandDraw() {
    try {
        ...
        totalHands++;
		var xmlString = await loadXMLDoc("./xml/mulligan.xml");
        ...
        updateValues(totalLandsInDeck, formattedMulliganRate, totalHands, totalMulligans, mulliganPercentage);
...
function updateValues(totalLandsInDeck, formattedMulliganRate, totalHands, totalMulligans, mulliganPercentage) {
...
function getMulliganRate(xmlDoc, totalLands) {
    ...
    for (var i = 0; i < landElements.length; i++) {
        var landElement = landElements[i];
        var quantityElement = landElement.getElementsByTagName("Quantity")[0];

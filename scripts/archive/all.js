...
function start() {
  ...
  mtgDeck.push('./xml/CloudpostWelder.xml', './xml/BlackDread.xml');
...
function displayDeck() {
  ...
  const deckInfoNode = createDeckInfoNode(
    deckListName,
    deckInfo.convertedCost,
    deckInfo.landCount,
    deckInfo.creatureCount
  );
...
function getMulliganRate(intTotLands) {
  ...
  document.getElementById('HypergeometricDistribution').innerHTML =
    'Hypergeometric Distribution(60 cards): ' + strMulligan;
  return strMulligan;
}
...
function getMulliganStats() {
  ...
  document.getElementById('MulliganPercentage').innerHTML =
    'Current Mulligan Percentage: ' + parseInt((intNumMulligans / lastRow) * 100) + '%';
}
